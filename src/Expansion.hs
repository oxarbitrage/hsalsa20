...
expand32Compute k0 k1 n = do
    if length k0 == 16 && length k1 == 16 && length n == 16 then do
        salsa20Compute $ sort32Compute k0 k1 n
    else
        error "inputs to `expand32Compute` must be 2 lists of 16 `Word32` numbers as a k0 and k1; and a \
...
expand32Display k0 k1 n = do 
    if length k0 == 16 && length k1 == 16 && length n == 16 then do
        salsa20Display $ sort32Display k0 k1 n
    else
        error "inputs to `expand32Display` must be 2 lists of 16 `String` strings as a k0 and k1;  and a \
...
expand32Equations k0 k1 n = do
    if length k0 == 16 && length k1 == 16 && length n == 16 then do
        let display = expand32Display k0 k1 n
        let displayIndex = zip [index0..] display
        let equation = map (uncurry (printf "z%d = %s")) displayIndex
        equation
    else
        error "inputs to `expand32Equations` must be 2 lists of 16 `String` strings as a k0 and k1;  and a \
            \list of 16 `String` strings as an `n`"
...
expand16Compute k n = do
    if length k == 16 && length n == 16 then do
        ...
        error "inputs to `expand16Compute` must be a list of 16 `Word32` numbers as a key and a \
            \list of 16 `Word32` numbers as an `n`"

-- |The expansion function displayed where we have one 16 bytes (k).
expand16Display :: [String] -> [String] -> [String]
expand16Display k n = do 
    if length k == 16 && length n == 16 then do
        salsa20Display $ sort16Display k n
    else
        error "inputs to `expand16Display` must be a list of 16 `String` strings as a key and a \
            \list of 16 `String` strings as an `n`"

-- |The expansion function equations where we have one 16 bytes (k).
expand16Equations :: [String] -> [String] -> [String]
expand16Equations k n = do
    if length k == 16 && length n == 16 then do
        let display = expand16Display k n
        let displayIndex = zip [index0..] display
        let equation = map (uncurry (printf "z%d = %s")) displayIndex
        equation
    else
        error "inputs to `expand16Equations` must be a list of 16 `String` strings as a key and a \
            \list of 16 `String` strings as an `n`"
