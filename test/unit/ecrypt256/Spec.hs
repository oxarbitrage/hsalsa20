{-|
ECRYPT tests for the 256 bit key size.

TODO: 
- Add the xordigest tests.
- This file is almost the same code as `ecrypt128/Spec.hs`. We should refactor it.
-}
...
main :: IO ()
main = do
    putStrLn "-- Salsa20 ECRYPT 256 bit key test vectors --"
    putStrLn ""

    -- Read the 256 bit key size json.
    json_file <- B.readFile "test/unit/ecrypt256/test_vectors_256.json"
